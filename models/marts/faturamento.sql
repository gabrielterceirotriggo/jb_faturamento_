{{
    config(
        ...
        post_hook=[
            "{{update_reversao() }}","{{atualiza_ordem()}}","{{vlr_cont_delta()}}"
        ]
    )
}}
SELECT
...
SELECT
    ...
    documento_calculo_anterior,
    estrutura_regional_politica,
    CAST(NULL AS TIMESTAMP_NTZ(0)) AS data_dados,
    CAST(ORDEM_FATURAMENTO AS NUMBER(38,0)) AS ORDEM_FATURAMENTO,
    CAST(CONSUMO_REGISTRADO AS NUMBER(28,14)) AS CONSUMO_REGISTRADO,
    eusdb
FROM 
    {{ ref ('int_estornados_delta')}}

{% if is_incremental() %}
WHERE
    mes_competencia NOT IN (
        SELECT mes_competencia
        FROM {{ this }}
    )
    AND documento_calculo NOT IN (
        SELECT documento_calculo
        FROM {{ this }}
    )
    AND documento_impressao NOT IN (
        SELECT documento_impressao
        FROM {{ this }}
    )
{% endif %}